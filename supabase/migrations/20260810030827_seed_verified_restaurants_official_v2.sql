with verified(slug,name,hood,address,postal,phone,website,source_url) as (
  values
    ('audrey','Audrey','east-nashville','809 Meridian Street','37207','615-988-3263','https://www.audreynashville.com/','https://www.audreynashville.com/contact'),
    ('noko','Noko','east-nashville','701 Porter Rd','37206','615-712-6894','https://www.nokonashville.com/','https://www.nokonashville.com/location/noko-nashville/'),
    ('the-optimist','The Optimist','germantown','1400 Adams Street','37208','615-709-3156','https://www.theoptimistrestaurant.com/nashville','https://www.theoptimistrestaurant.com/nashville'),
    ('husk-nashville','Husk Nashville','sobro','37 Rutledge St.','37210','615-256-6565','https://husknashville.com/','https://husknashville.com/')
)
...
with source as (select id from public.data_sources where provider_key='official_website'), verified(slug,website,source_url) as (values ('audrey','https://www.audreynashville.com/','https://www.audreynashville.com/contact'),('noko','https://www.nokonashville.com/','https://www.nokonashville.com/location/noko-nashville/'),('the-optimist','https://www.theoptimistrestaurant.com/nashville','https://www.theoptimistrestaurant.com/nashville'),('husk-nashville','https://husknashville.com/','https://husknashville.com/'))
...
with source as (select id from public.data_sources where provider_key='official_website'), targets as (select id from public.places where slug in ('audrey','noko','the-optimist','husk-nashville'))
...
select id,80,now(),false,null,now() from public.places where slug in ('audrey','noko','the-optimist','husk-nashville')
