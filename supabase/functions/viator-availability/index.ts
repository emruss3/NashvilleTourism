...
/**
 * NashRoam -> Viator availability boundary for the REAL-TIME SEARCH MODEL.
 *
 * Allowed provider calls:
 * - GET /availability/schedules/{product-code}: one selected product only
 * - POST /availability/check: only after the user selects a specific date and
 *   passenger mix (Viator age bands). Never used for ingestion.
 *
 * NashRoam remains an affiliate implementation: checkout/payment/booking are
 * completed on Viator, not through Viator booking endpoints on NashRoam.
 */
...
function normalizeUnavailableDates(value: any) {
  ...
  return value
    .map((item: any) => {
      if (typeof item === "string") return dateString(item) ? { date: item } : null;
      const date = dateString(item?.date);
      return date
        ? { date, reason: typeof item?.reason === "string" ? item.reason : undefined }
        : null;
    })
    .filter(Boolean);
}
...
function normalizeRetailPrice(price: any) {
  ...
  return {
    original,
    special,
    offerStartDate: dateString(price?.special?.offerStartDate),
    offerEndDate: dateString(price?.special?.offerEndDate),
  };
}
                        ...
                        .map((record: any) => {
                          ...
                          const pricingDetails = Array.isArray(record?.pricingDetails)
                            ? record.pricingDetails
                                .map((detail: any) => {
                                  const price = normalizeRetailPrice(detail?.price);
                                ...
                                .filter(Boolean)
                            : [];
                          return { daysOfWeek, timedEntries, pricingDetails };
                        })
                        .filter(Boolean)
                    : [];
                  return {
  ...
  const paxMix = Array.isArray(input?.paxMix)
    ? input.paxMix
        .map((item: any) => {
          ...
          if (
            !Number.isFinite(numberOfTravelers) ||
            numberOfTravelers < 1 ||
            numberOfTravelers > 99
          ) {
            return null;
          }
...
function normalizeAvailabilityCheck(data: any) {
  ...
  return {
    ...
    productOptionCode:
      typeof data?.productOptionCode === "string" ? data.productOptionCode : undefined,
...
Deno.serve(async (req: Request) => {
  ...
  try {
      ...
      return json(
        {
          ...
          supported: ["get_schedules", "check_availability"],
    ...
    if (mode === "check_availability") {
      ...
      const availability = result.ok ? normalizeAvailabilityCheck(result.data) : null;
