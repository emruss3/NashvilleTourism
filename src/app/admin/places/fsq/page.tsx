...
export const metadata = {
  title: 'Legacy FSQ Staging | Nashroam Admin',
  ...
  return (
    <main className="shell pb-20 pt-10">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-paper-edge pb-6">
        <div>
          <p className="eyebrow">Internal · Legacy</p>
          <h1 className="mt-2 font-display text-3xl font-bold text-navy md:text-4xl">FSQ OS staging</h1>
          <p className="mt-3 max-w-3xl text-sm leading-relaxed text-ink-soft">
            Legacy Foursquare OS promotion UI. New discovery should land in the provider-agnostic queue at{' '}
            <Link href="/admin/places" className="font-semibold text-navy underline-offset-4 hover:text-clay hover:underline">/admin/places</Link>.
            When FSQ portal access is available, FSQ deltas should feed <code className="font-mono">place_discovery_candidates</code>.
          </p>
          <div className="mt-4 flex flex-wrap gap-4 text-sm font-semibold">
            <Link href="/admin/places" className="text-navy underline-offset-4 hover:text-clay hover:underline">Discovery</Link>
            <span className="text-ink-faint">Legacy FSQ</span>
      ...
      {candidates.length === 0 ? (
        <section className="mt-8 rounded-card border border-paper-edge bg-paper-card p-7 shadow-card">
          <h2 className="font-sans text-xl font-bold text-navy">Waiting for FSQ OS import</h2>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-ink-soft">
            Prefer the Overture weekly sync for primary discovery. Use FSQ OS as a secondary enrichment/delta source when portal access is available.
          </p>
        </section>
      ) : (
                  ...
                  <form action={`/api/admin/places/${encodeURIComponent(item.fsq_place_id)}/promote`} method="post" className="mt-4 grid gap-4 md:grid-cols-2">
                    <div>
                      ...
                      <select name="primaryCategory" required defaultValue="" className="mt-1 min-h-[42px] w-full rounded-lg border border-paper-edge bg-white px-3">
                        ...
                        {[...new Set([...suggested, ...CATEGORY_OPTIONS])].map((cat) => (
