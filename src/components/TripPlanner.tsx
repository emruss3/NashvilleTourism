    ...
    <div>
      <form onSubmit={onSubmit} className="rounded-card border border-paper-edge bg-white p-5 lg:p-7">
        <div className="grid gap-6 sm:grid-cols-2">
          ...
          <div>
            <label htmlFor="tripType" className="field-label">
              Trip type
            </label>
            <select
              id="tripType"
              ...
              return (
                <label
                  ...
                  className={`cursor-pointer rounded-full border px-3.5 py-1.5 text-sm transition-colors ${
                    on ? 'border-clay bg-clay text-white' : 'border-paper-edge bg-white text-ink-soft hover:border-clay'
                  }`}
                >
                  <input
                    type="checkbox"
                    className="sr-only"
                    checked={on}
                    onChange={() => toggleArray('interests', opt)}
                  />
                  {opt}
                </label>
              );
              ...
              return (
                ...
                >
                  <input
                    ...
                    className="sr-only"
                    checked={on}
                    onChange={() => toggleArray('neighborhoods', n.slug as NeighborhoodSlug)}
                  />
                  {n.name}
                </label>
              );
          ...
          <div className="space-y-2.5">
            {(
              ...
              <label key={key} className="flex items-center gap-2.5 text-[15px] text-ink-soft">
                <input
                  type="checkbox"
                  className="h-4 w-4 rounded border-paper-edge text-clay focus:ring-clay"
                  checked={input[key]}
        ...
        <button type="submit" className="btn-primary mt-8 w-full sm:w-auto">
          {submitted ? 'Rebuild itinerary' : 'Build my itinerary'}
        </button>
        <p className="mt-3 text-2xs text-ink-faint">
          Plans are assembled from our published listings using set rules, not generated prose.
        </p>
      </form>
        ...
        <section id="itinerary-result" aria-live="polite" className="mt-12">
          <div className="flex flex-wrap items-end justify-between gap-4 border-b border-paper-edge pb-4">
            <div>
              ...
              <h2 className="text-3xl">
                {days} day{days === 1 ? '' : 's'} in Nashville
              </h2>
                    ...
                    <li key={`${stop.slot}-${i}`} className="rounded-card border border-paper-edge bg-white p-4">
                      ...
                      <dl className="mt-3 space-y-1 text-sm">
                        ...
                        {stop.reservationNote && (
                          <div className="flex gap-2">
                            ...
                            <dd className="text-ink-soft">{stop.reservationNote}</dd>
                          </div>
                        )}
                      </dl>
                      ...
                      {stop.alternatives.length > 0 && (
                        <details className="mt-3 border-t border-paper-edge pt-3">
                          <summary className="cursor-pointer text-sm font-semibold text-ink-soft hover:text-clay">
                            Alternatives ({stop.alternatives.length})
                          </summary>
                          <ul className="mt-2 space-y-2">
                            {stop.alternatives.map((alt) => (
                              <li key={alt.title} className="text-sm">
                                {alt.href ? (
                                  <Link href={alt.href} className="font-semibold text-clay hover:underline">
                                    {alt.title}
                                  </Link>
                                ) : (
          ...
          {hotelPicks.length > 0 && (
            <section className="mt-12 border-t border-paper-edge pt-8">
              ...
              <p className="mt-1 text-[15px] text-ink-soft">
                Matched to your neighborhoods and budget.
              </p>
